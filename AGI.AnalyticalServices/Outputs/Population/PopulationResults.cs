...
namespace AGI.AnalyticalServices.Outputs.Population
{
    public class PopulationResults
    {
        ...
        public PopulationResults(Dictionary<string,PopulationAccumulator> resultData)
        {
            PopulationValues = new List<double>();
            Weights = new List<double>();
            foreach (var id in resultData.Keys)
            {
                Weights.Add(resultData[id].Count);
                PopulationValues.Add(resultData[id].PopulationValue);
            }
        }
        /// <summary>
